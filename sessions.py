...
class SessionManager:
    ...
    def cleanup(self, sid):
        ...
        # Signal Voice Live shutdown
        shutdown_event = state.get("vl_shutdown_event")
        if shutdown_event:
            shutdown_event.set()

        vl_thread = state.get("vl_thread")
        if vl_thread and vl_thread.is_alive():
            try:
                vl_thread.join(timeout=3.0)
            except Exception:
                pass
